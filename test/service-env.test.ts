...
describe("service environment", () => {
  ...
  describe("start-services behavior", () => {
    it("starts without messaging-related warnings", () => {
      ...
      // Prepend a no-op cloudflared shim so the script's cloudflared check
      // finds something but the process exits immediately — avoiding the 15-second
      // tunnel wait loop on machines that have cloudflared installed.
      const shimDir = mkdtempSync(join(tmpdir(), "nemoclaw-shim-"));
      try {
        // Output a trycloudflare URL so get_tunnel_url() finds it immediately
        // if the wait loop runs, regardless of whether the process is still alive.
        writeFileSync(
          join(shimDir, "cloudflared"),
          "#!/bin/sh\necho 'https://test-shim.trycloudflare.com'\nexit 0\n",
          { mode: 0o755 },
        );
  ...
  describe("CLOUDFLARE_TUNNEL_TOKEN", () => {
    ...
    it("CLOUDFLARE_TUNNEL_TOKEN is preserved when set", () => {
      const result = execSync(`bash -c '${getTokenLine}; echo "$CLOUDFLARE_TUNNEL_TOKEN"'`, {
        encoding: "utf-8",
        env: { ...process.env, CLOUDFLARE_TUNNEL_TOKEN: "eyJtest" },
      }).trim();
    ...
    it("get_tunnel_url parses hostname from named tunnel JSON config log line", () => {
      const piddir = mkdtempSync(join(tmpdir(), "test-named-"));
      try {
        // cloudflared logs named-tunnel ingress as JSON-escaped config= value:
