...
describe("CLI dispatch", () => {
  ...
  it("start does not prompt for NVIDIA_API_KEY before launching local services", () => {
    ...
    // Shim cloudflared so the 15-second wait loop exits immediately on
    // machines where the real cloudflared binary is installed.
    // Outputting a trycloudflare URL ensures getTunnelUrl() finds it at once.
    fs.writeFileSync(
